...
def _schema_fields() -> list[str]:
    fields = getattr(WorkOrder, "model_fields", None)
    if fields is None:
        fields = WorkOrder.__fields__
...
def load_wo(file_path: str) -> list[WorkOrder]:
    """Load work orders from a CSV or Excel file into schema objects."""
    ...
    for _, row in df.iterrows():
        ...
        records.append(WorkOrder(**values))
